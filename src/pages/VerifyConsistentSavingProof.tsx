...
const VERIFIER_CONTRACT_ID =
  import.meta.env.VITE_CONSISTENT_SAVING_VERIFIER_CONTRACT_ID ||
  "CDBUSPDUC4AYSWPVCX5QQLRVFJUEJDW3BTZ5EOVH77TCJXW3CKC5X6KQ";
...
const VerifyConsistentSavingProof = () => {
  ...
  useEffect(() => {
    ...
    async function fetchRecord() {
      try {
        ...
        // Read-only simulation: call get_proof(nullifier) — no signing needed
        ...
        if (!account) {
          ...
          const rec = native as { user: string; months_with_saving: number; threshold_months: number; ledger: number };
          ...
          <div className="space-y-4">
            ...
            <div className="bg-muted rounded-sm p-4 space-y-3 font-mono text-xs">
              ...
              <div className="flex justify-between gap-2">
                ...
                <span className="text-primary font-bold">
                  ≥ {state.record.threshold_months} de 12
                </span>
              </div>
              <div className="flex justify-between gap-2">
            ...
            <div className="p-3 bg-muted rounded-sm">
              <p className="text-[0.65rem] text-muted-foreground font-mono leading-relaxed">
                Esta prueba garantiza que la cuenta indicada ahorró en al menos{" "}
                {state.record.threshold_months} de los últimos 12 meses, aproximadamente el{" "}
                {state.record.ledgerDate ?? "día registrado"}. Los montos exactos nunca fueron revelados.
              </p>
            </div>
          </div>
        )}
...
export default VerifyConsistentSavingProof;
